...
class SimulationPlots:
    ...
    def plot_mean_unloaded_planes(
        ...
        for scenario_num, planes in scenarios.items():
            mean_planes = [AirPlane.calculate_mean_unloaded_rate(
                planes, 0, window_end, window_size) for window_end in time_windows]
            ax.plot(time_windows, mean_planes, label=f"{scenario_num} {scenario_label}", marker=".", markersize=4)
    ...
    def plot_mean_queue_length(
        ...
        for scenario_num, planes in scenarios.items():
            mean_queue_lengths = [AirPlane.calculate_mean_queue_length(
                planes, 0, window_end) for window_end in time_windows]
            ax.plot(time_windows, mean_queue_lengths, label=f"{scenario_num} {scenario_label}", marker=".", markersize=4)
    ...
    def plot_mean_waiting_time(
        ...
        for scenario_num, planes in scenarios.items():
            mean_waiting_times = [AirPlane.calculate_mean_waiting_time(
                planes, 0, window_end) for window_end in time_windows]
            ax.plot(time_windows, mean_waiting_times, label=f"{scenario_num} {scenario_label}", marker=".", markersize=4)
    ...
    def plot_mean_robot_utilization(
        ...
        for scenario_num, planes in scenarios.items():
            mean_utilization_rates = [
                AirPlane.calculate_mean_robot_utilization(planes, 0, window_end)
                for window_end in time_windows
            ]
            ax.plot(time_windows, mean_utilization_rates, label=f"{scenario_num} {scenario_label}", marker=".", markersize=4)
    ...
    def plot_all_metrics(
        ...
        window_size: int = 600,
        scenario_label: str = "robots",
        output_filename: str = "all_metrics_tp1.png"
    ) -> None:
        """
        ...
        - scenario_label (str): Label to use for scenarios in the legend (e.g., "robots" or "minutes")
        - output_filename (str): Name of the output file to save the plot
        """
        _, ax_unloaded = SimulationPlots.plot_mean_unloaded_planes(scenarios, simulation_duration, window_size, scenario_label)
        _, ax_queue = SimulationPlots.plot_mean_queue_length(scenarios, simulation_duration, window_size, scenario_label)
        _, ax_waiting = SimulationPlots.plot_mean_waiting_time(scenarios, simulation_duration, window_size, scenario_label)
        _, ax_utilization = SimulationPlots.plot_mean_robot_utilization(
            scenarios, simulation_duration, window_size, scenario_label)
