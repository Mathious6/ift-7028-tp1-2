...
def main():
    ...
    for num_robots in list(SimulationConfig.ROBOT_SCENARIOS.keys()):
        ...
        unloaded_planes = sum(
            ...
            if plane.status == airport.planes[0].status.UNLOADED
        )
        queue_waiting_times = [
            plane.waiting_time
            for plane in airport.planes
        ...
        root_logger.info(
            f"Scenario execution time: {time.time() - start_time:.2f} seconds"
        )
    ...
    SimulationPlots.plot_robot_utilization(scenarios, SIMULATION_DURATION, WINDOW_SIZE)
